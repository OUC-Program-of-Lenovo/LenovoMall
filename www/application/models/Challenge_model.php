...
class Challenge_model extends CI_Model {
    ...
    public function get_challenge_solved_user_id($challenge_id)
    {
    ...
    function formatTime($time){
        ...
        if ($time < 60){
            ...
            if($d === 1){
                ...
                $str = ' the day before yesterday '.$htime;
            }
        }else{
            $str = $rtime;
        }
    ...
    public function get_challenge_socre($challenge_id)
    {
        $query = $this->db->get_where('challenges', array('challenge_id' => $challenge_id));
        $result = $query->row_array();
        return $result['score'];
    ...
    public function is_challenge_online($challenge_id)
    {
        $this->db->select('fixing');
        $query = $this->db
            ->where(array(
                'challenge_id' => $challenge_id,
            ))
    ...
    public function is_challenge_name_existed($challenge_name)
    {
        $query = $this->db->get_where('challenges', array('name' => $challenge_name));
        return ($query->num_rows() > 0);
    }
}